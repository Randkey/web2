...
# --- Список книг (без изменений логики) ---
@bp.route('/')
def index():
    try:
    ...
    db = get_db()
    total = db.execute('SELECT COUNT(*) as cnt FROM books').fetchone()['cnt']
    ...
    LIMIT ? OFFSET ?
    """
    books = db.execute(query, (per_page, offset)).fetchall()
...
def book_edit(book_id):
    ...
    genres_all = db.execute('SELECT id, name FROM genres ORDER BY name').fetchall()
    current_genres = [str(row['genre_id']) for row in db.execute('SELECT genre_id FROM book_genres WHERE book_id = ?', (book_id,)).fetchall()]
    ...
    if request.method == 'POST':
        ...
        if not title or not short_description or not year or not publisher or not author or not pages:
            flash('Заполните все обязательные поля', 'error')
            return render_template('book_form.html', genres=genres_all, form=request.form, action='edit', book=book, current_genres=current_genres)
    ...
    return render_template('book_form.html', genres=genres_all, form=book, action='edit', book=book, current_genres=current_genres)
